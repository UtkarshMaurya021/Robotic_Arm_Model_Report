...
// SERVO CONNECTIONS
// Neck servo - D10 - pos is saved at EEPROM 3
...
// All the gripper positions will be saved and read from EEPROM
// to resume the same position
...
// Function to control the servo based on joystick position
void control_servo(Servo &current_servo, int current_pos, int EEPROM_addr)
{
    // Position should always be between 0 to 180
    if (current_pos >= 180)
    ...
    current_servo.write(current_pos); // update servo position
    EEPROM.write(EEPROM_addr, current_pos); // save position in EEPROM

    Serial.print(EEPROM_addr);
    Serial.print(" = ");
    Serial.println(current_pos); // for debugging on serial monitor
}
...
void setup()
{
    ...
    UpDown_servo.write(updown_pos);

    FrontBack_servo.attach(11);
    FrontBack_servo.write(frontback_pos);
...
void loop()
{
    ...
    Serial.print(" ,");
    Serial.print(updown_pos);
    Serial.print(" ,");
    Serial.print(frontback_pos);
    Serial.print(" ,");
    Serial.print(neck_pos);
    Serial.println(".");
    */
